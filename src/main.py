...
Let's get some audio data!  How would you like to choose your tracks:
  ...
  3.) Enter playlist id(s)
  4.) Pick from your saved songs
  """)
        ...
        if program_choice == '1':
            ...
            playlistids = input('\nPlease enter playlist ids separated by comma(,) eg: 37i9dQZF1DX186v583rmzp:')
            selected_tracks = list_tracks_by_playlist_id(spotify, playlistids)
        ...
        ("tracks" if len(selected_tracks) > 1 else "track")))
            ...
            if display_choice == '1':
                audio_features = get_audio_features(
                    spotify, selected_tracks, pretty_print=False,save_as_json=True)
               # input("""
                #Do you want to use this data for likeability analysis?
                 # 1.) Choose LHS songs
                  #2.) Choose RHS songs
                  #""")
                #analysis_choice = input('Choice: ')
                #if display_choice == '1':
                    #save_audio_features_to_json()
...
def get_audio_features(spotify, tracks, pretty_print=False,save_as_json=False):
    """
    ...
    #print(tracks_features_response)
    ...
    if save_as_json:
        filename = input('\nPlease enter json filename you want to save the data to:')
        with open(filename, 'w', encoding='utf-8') as f:
            json.dump(track_features_map, f, ensure_ascii=False, indent=4)
        #print(track_features_map)
...
def list_tracks_by_playlist_id(spotify, playlistid):
    """
    Get all of a user's playlists and have them select tracks from a playlist
    """
    ...
    # The API paginates the results, so we need to keep fetching until we have all of the items
    while len(tracks) < total:
        tracks_response = spotify.playlist_tracks(playlistid, offset=len(tracks))
        tracks.extend(tracks_response.get('items', []))
        total = tracks_response.get('total')
    ...
    # Print out our tracks along with the list of artists for each
    print_header('Tracks in "{}"'.format(playlistid))
...
#def save_audio_features_to_json():
